...
import { Button, Select, MenuItem, Box, Typography, TextField, CircularProgress, Skeleton, Alert } from '@mui/material';
...
function DashboardReports() {
  ...
  const handleDetailClick = async (strategyName) => {
    ...
    router.push({
      pathname: `stock-reports/${strategyName}`,
      query: queryParams
    });
  };
  ...
  const handleFilter = async () => {
    ...
    try {
      let url = `${baseUrl}ema-scalping/volume-pnl-report/?`;
              ...
              {loading ? (
                ...
                <>
                  ...
                  <Typography
                    sx={{
                      color:
                        parseFloat(totalPnl) > 0 ? 'success.main' : parseFloat(totalPnl) < 0 ? 'error.main' : 'inherit',
                      fontWeight: 500
                    }}
                    fontWeight="600"
                  >
              ...
              <TableBody>
                {loading ? (
                  ...
                  filterData.map((row, index) => (
                    ...
                    >
                      <TableCell>{row?.strategy_name || row?.strategyName}</TableCell>
                      <TableCell
                        sx={{
                          color:
                      ...
                      >
                        {typeof row?.pnl === 'number' ? row.pnl.toFixed(2) : row?.pnl}
                      </TableCell>
                      <TableCell
                        sx={{ cursor: 'pointer', textDecoration: 'underline' }}
                        onClick={() => handleDetailClick(row?.strategy_name || row?.strategyName)}
                      >
                        Detail
                      </TableCell>
                    </TableRow>
