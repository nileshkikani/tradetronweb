...
function DashboardDeployedContent() {
  ...
  const [selectedStrategyForDeactivation, setSelectedStrategyForDeactivation] =
    useState(null);
  const [selectedStrategyForDeletion, setSelectedStrategyForDeletion] =
    useState(null);
  const [isLoadingActivateDeactivate, setIsLoadingActivateDeactivate] =
    useState(null);
    ...
    if (selectedStrategyId && selectedParam) {
      try {
        const { data } = await axiosInstance.get(
          API_ROUTER.ORDER_LIST(selectedStrategyId, selectedParam),
          { headers }
        );
        setOrderList(data);
  ...
  const confirmLock = async () => {
    try {
      ...
      showToast(
        error?.response?.data?.details || TOAST_ALERTS.GENERAL_ERROR,
        TOAST_TYPES.ERROR
      );
    } finally {
      setIsLoading(false);
  ...
  const handleSymbolClick = async (order) => {
    ...
    if (selectedStrategyId && selectedDate) {
      try {
        const { data } = await axiosInstance.get(
          API_ROUTER.ORDER_LIST(selectedStrategyId, selectedDate, positionType),
          { headers }
        );
    ...
    debounce(async (strategyId, currentStatus) => {
      ...
      if (currentStatus) {
        ...
        try {
          ...
          showToast(
            response.data.details || TOAST_ALERTS.STRATEGY_STATUS,
            TOAST_TYPES.SUCCESS
          );
  ...
  return (
    <Box sx={{ height: "100vh", overflow: "hidden", overflowY: "auto" }}>
          ...
          <>
            ...
            {orderList.length > 0 && (
              ...
              <TableContainer
                component={Paper}
                elevation={0}
                sx={{ borderRadius: "8px", overflow: "hidden",margin: '0',maxWidth: '98%',marginTop:"10px"   }}
              >
                <Box
                  sx={{
                    display: "flex",
                  ...
                  <Typography variant="subtitle1" fontWeight="600">
                    ...
                    <Box
                      ...
                      sx={{
                        ml: 1,
                        color: (theme) => {
                          const totalPL = orderList.reduce((total, order) => {
                            const closePrice =
                              order.close_price === null
                                ? getLivePrice(order.token)
                                : order.close_price;
                            const profit =
                              order.order_type === "SELL"
                                ? (order.open_price - closePrice) *
                                  order.quantity
                      ...
                      : orderList.map((order) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <TableCell
                                onClick={() => handleSymbolClick(order)}
                                style={{
                                  cursor: "pointer",
                                  color: "lightblue",
          ...
          <Box>
            <TableContainer
              ...
              sx={{
                ...
                msOverflowStyle: "none", // To hide scrollbar in IE and Edge
      ...
      >
        ...
        <DialogContent>
          <Typography>
            All open orders with this strategy will be closed. Are you sure you
            want to deactivate this strategy?
          </Typography>
        </DialogContent>
        <DialogActions>
          <Button
          ...
          <Button
            ...
            color="error"
            disabled={
              isLoadingActivateDeactivate?.[selectedStrategyForDeactivation]
            } // Ensure safe access
          >
            Deactivate
          </Button>
